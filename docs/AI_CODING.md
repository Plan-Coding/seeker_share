...
>
> **服务与测试约定**:未收到用户的明确测试指令,AI 不得擅自启动/重启服务或进行运行时测试;代码改动后完成静态检查与单元测试、确认编译无误后,通知用户自行启动并测试。
...
3. **修复**:遵循 `AGENTS.md` 中的工程约定;工具箱新增工具保持纯前端、不加载外部网络依赖。
...
【功能要求】……
【约束】
- 纯前端实现、不从外部网络/CDN 加载依赖(第三方库需下载到 `static/vendor/` 本地引入并保留许可证)
- 遵循 TOOLS 数组注册规范
- 数据不出本机
【验收标准】……
...
| 2026-08-30 | 技能体系 | 新增 `.agents/skills/` 分层技能目录(3 个技能),并在 `AGENTS.md` 指明 |
