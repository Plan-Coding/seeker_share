...
> 一个轻量、炫酷、基于 H2 的局域网消息与文件共享节点，内置 28 个纯前端运维小工具。
...
顶部导航切换到「工具箱」，28 个常用小工具**全部在浏览器本地运行，零后端依赖，数据不出本机**：
...
| 解析校验 | JSON 格式化 / 压缩 / 键名排序、YAML ⇄ Properties 配置互转（嵌套 / 数组 / 注释）、XML 格式化 / 压缩 / 校验（保留注释与 CDATA）、正则测试器（实时高亮、捕获分组、速查表）、Cron 解析器（5/6 位表达式、中文释义、推算未来执行时间）、时间戳转换、URL 解析 |
...
- 原生 JavaScript / CSS3（工具箱不加载外部网络依赖、需时本地 vendor；文档库使用自托管 Yjs / markdown-it / highlight.js）
- Maven Wrapper
